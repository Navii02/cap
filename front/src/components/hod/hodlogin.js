...
function HodLogin() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch(`${baseurl}/api/hodlogin`, {
        method: 'POST',
        headers: {
      ...
      if (data) {
        ...
        if (!isMounted.current) return;
        alert('Login failed.Please contact Office');
        window.location.href = '/faculityhome';
      }
        ...
        <div className="login-form">
          ...
          <form className="login-fill" onSubmit={handleSubmit}>
            ...
            <div className="signup-link">
              <p>
                Not yet registered? <Link to="/hodsignup">Sign up</Link>
              </p>
            </div>
