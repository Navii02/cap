...
function useTutorSignup() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch(`${baseurl}/api/classtutorregister`, {
        method: 'POST',
        headers: {
...
function TutorSignup() {
  const { values, errors, handleChange, handleShowPassword, handleSubmit } = useTutorSignup();
        ...
        <div className="signup-form">
          ...
          <form className='signup-fill' onSubmit={handleSubmit} noValidate>
            ...
            <div className="login-link">
              <p>Already have an account? <Link to="/classtutorlogin">Login</Link></p>
            </div>
          </form>
        </div>
      </div>
